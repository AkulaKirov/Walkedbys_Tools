...
Public Class 创意工坊合集物品批量添加
    ...
    Private Sub ButStart_Click(sender As Object, e As EventArgs) Handles ButStart.Click
        ...
        TH = New Thread(Sub()
                            ...
                            Dim mc As MatchCollection = Regex.Matches(TxtInput.Text, "\([0-9]{3,}\)"), ht As Date = Now
                            If mc.Count > 0 Then
                                For Each m As Match In mc
                                    ...
                                    TxtOut.Text += vbCrLf
                                Next
                                If g + g2.Count > 0 Then
                                    TxtOut.Text += "一共导入成功：" + g.ToString + "个，失败：" + g2.Count.ToString + "个。"
                                    TxtOut.Text += vbCrLf + "用时：" + 时间差(ht, Now, True) + vbCrLf
                                    If g2.Count > 0 Then
                                        TxtOut.Text += "失败物品列表：" + vbCrLf
                                        For Each n In g2
                                            TxtOut.Text += 括(n) + vbCrLf
                                        Next
                                    End If
                                End If
                            Else
    ...
    Private Sub TxtID_TextChanged(sender As Object, e As EventArgs) Handles TxtID.TextChanged, TxtInput.TextChanged
        ...
        ButStart.Enabled = 文本框都有字(TxtID, TxtInput)
    End Sub
