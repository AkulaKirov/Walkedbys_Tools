...
Public Class 未响应图片制作器
    ...
    Private Sub 未响应图片制作器_Paint(sender As Object, e As PaintEventArgs) Handles Me.Paint
        ...
        Select Case ListStyle.SelectedIndex
            Case 0
                g.DrawString(名字, New Font("Segoe UI", 9), New SolidBrush(Color.Black), 31, 9)
                g.DrawString(名字 + " has stopped working", New Font("Segoe UI", 12), New SolidBrush(Color.FromArgb(0, 51, 153)), 58, 43)
            ...
            Case 2
                ...
                End If
        End Select
        If Not IsNothing(输出) Then e.Graphics.DrawImage(输出, 12, 81, 输出.Width, 输出.Height)
    End Sub
